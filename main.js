function getOpheliaApiBaseUrl() {
  const configured = window.OPHELIA_CONFIG?.apiBaseUrl || window.OPHELIA_API_BASE_URL || "";
  if (configured) return String(configured).trim().replace(/\/$/, "");
  const localHosts = ["localhost", "127.0.0.1", ""];
  return localHosts.includes(window.location.hostname) ? "http://localhost:5159/api" : "";
}

const API_BASE_URL = getOpheliaApiBaseUrl();
